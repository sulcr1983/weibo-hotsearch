"""可观测性模块 V4.1：漏斗日志、Trace ID、过滤原因、失败快照、HTML断言"""
import json
import os
import re
import uuid
...
class FunnelStats:
    ...
    def record_trace(self, trace_id: str, stage: str, detail: dict = None):
        if trace_id not in self.traces:
            self.traces[trace_id] = {'stages': [], 'detail': {}}
        self.traces[trace_id]['stages'].append(stage)
        if detail:
            self.traces[trace_id]['detail'].update(detail)
...
CAPTCHA_PATTERNS = [
    ...
    re.compile(r'请输入验证码', re.IGNORECASE),
...
LOGIN_PATTERNS = [
    ...
    re.compile(r'请登录后查看', re.IGNORECASE),
...
def check_html_validity(html: str, source_name: str = '') -> dict:
    """检查HTML有效性，返回 {'valid': bool, 'issues': [str], 'issue_type': str|None}"""
    if not html:
