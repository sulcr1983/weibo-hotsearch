...
| 痛点 | 系统对策 |
|------|---------|
| 信息源太多，看不过来 | 一键接入 18 个数据源（11 RSS + 7 网页直采），每小时轮询 |
...
| 品牌 | 匹配关键词 | 品牌色 |
|------|-----------|--------|
| 小米汽车 | 小米汽车 / 小米SU7 / 小米SU / 小米YU7 | `#FF6900` |
| 鸿蒙智行 | 问界 / 智界 / 尊界 / 享界 / 尚界 / 鸿蒙智行 | `#CE0E2D` |
| 零跑汽车 | 零跑 / 零跑C | `#0052D9` |
| 理想汽车 | 理想汽车 / 理想L / 理想MEGA / 理想i / 理想ONE | `#4A90D9` |
| 蔚来汽车 | 蔚来 / 萤火虫 / 乐道 | `#2E6BE6` |
| 极氪汽车 | 极氪 / 极氪00 | `#00B2A9` |
| 阿维塔 | 阿维塔 | `#6C5CE7` |
| 智己汽车 | 智己 / 智己L | `#E84393` |
| 比亚迪 | 比亚迪 / 仰望 / 腾势 / 方程豹 | `#C0392B` |
| 特斯拉 | 特斯拉 / Tesla / Model Y / Model 3 / Cybertruck / FSD | `#E74C3C` |

## 五、数据源清单

### 5.1 RSS 源（11 个，YAML 配置）

| 类别 | 来源 |
|------|------|
| 科技商业 | 36氪、钛媒体 |
| RSSHub 聚合 | 第一财经快讯、虎嗅、知乎日报、36氪快讯、财联社深度、财联社热门、澎湃新闻精选、财新最新、华尔街见闻 |
...
# ── 微博热搜 Cookie（浏览器登录后从控制台复制）──
WEIBO_COOKIE=SUB=_2A25...
...
```bash
# 查看服务状态
sudo systemctl status car-monitor-v3

# 查看实时日志
journalctl -u car-monitor-v3 -f

# 重启服务
sudo systemctl restart car-monitor-v3
...
| 现象 | 原因 | 解决 |
|------|------|------|
| 服务反复重启 | Python 导入失败 | `journalctl -u car-monitor-v3 -n 50` 查看错误 |
| 邮件发送失败 | QQ 授权码过期 | 重新获取 QQ 邮箱授权码，更新 `.env` |
| 微博抓取失败 | Cookie 过期 | 重新登录微博，复制新 Cookie 到 `.env` |
...
│   ├── sources/                  # 数据源统一配置
│   │   ├── __init__.py           # YAML 加载器
│   │   └── sources.yml           # 22 个数据源（增删改只需改 YAML）
│   ├── v2/                       # 共享模块
...
## 十三、从 V4.0 到 V4.1 的关键变化

| 维度 | V4.0 | V4.1 |
|------|------|------|
| 微博采集 | 直接API（需Cookie，已失效） | `weibo.com/ajax/side/hotSearch`（免Cookie，无需登录） |
| 微博调度 | 每 60 分钟 | 每 120 分钟（低频避免封禁） |
| 微博去重 | 同标题30分钟去重 | 同品牌+同标题 24h 去重（同一热搜只保留一条） |
| 微博链接 | `word_scheme` 字段（不可靠） | 统一 `s.weibo.com/weibo?q=关键词` |
| 微博飞书卡片 | 逐条展示+重复 | 去重合并+热搜标签(爆/热/新/沸) |
| UGC 过滤器 | 含 `\d+小时前\|次阅读\|万阅读` 误杀正常新闻 | 仅保留 `\d+天前` + 自媒体水印（触角说/电车网界等） |
| 观点过滤器 | 全源启用+13个宽正则（误杀正常新闻） | 仅对虎嗅等深度媒体启用+7个强观点词 |
| `/collect` API | 仅触发新闻采集 | 同时触发新闻+微博采集 |
| 日报格式 | 逐条列举 | 品牌分组→四维度分组→40字摘要 |
| 测试覆盖 | 无集成测试 | 8项回归测试 + 采集→入库→推送全链路集成测试 |
